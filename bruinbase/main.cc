...
int main()
{
    ...
    for(int i=0; i <= 5; i++)
    {
      poopRid.pid = i*3; poopRid.sid = i;
      zeroPls = poop.insert(i, poopRid);
    ...
    int eid = -1;
    cout << "Found key=5 at eid: " << poop.locate(5, eid) << endl;

    //Test location by finding key with 6 (doesn't exist - should be -1)
    int eid2 = -1;
    poop.locate(6, eid2);
    cout << "Found key=2 at eid2: Error code: " << poop.locate(6, eid) << endl;
    ...
    poop.readEntry(2, key, rid);
    cout << "Entry at specified eid has key=" << key << " and pid=" << rid.pid << " and sid=" << rid.sid << endl;
    poop.readEntry(4, key, rid);
    cout << "Entry at specified eid has key=" << key << " and pid=" << rid.pid << " and sid=" << rid.sid << endl;
